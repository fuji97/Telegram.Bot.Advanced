...
/// <summary>
/// Self-contained copy of <c>Telegram.Bot.Advanced.Tests/Infrastructure/TestHttpMessageHandler.cs</c>: fake
/// transport for a real <see cref="TelegramBotClient"/>, queuing responses per Bot API method name (the last URI
/// segment, e.g. "getMe", "sendMessage"). Duplicated rather than referenced: the unit project is an
/// <c>Exe</c> test host, and a test-project-to-test-project reference would couple two independently-runnable
/// suites.
/// </summary>
public sealed class FakeTelegramApi : HttpMessageHandler {
    ...
    /// <summary>
    /// Builds a real <see cref="TelegramBotClient"/> backed by a fresh <see cref="FakeTelegramApi"/>.
    /// </summary>
    public static TelegramBotClient CreateClient(out FakeTelegramApi api) {
        api = new FakeTelegramApi();
        var httpClient = new HttpClient(api);
        return new TelegramBotClient(new TelegramBotClientOptions("123456789:TEST-TOKEN"), httpClient);
    }
}
