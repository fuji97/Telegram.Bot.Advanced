...
public class NewsletterController<TContext>(INewsletterService newsletterService) : TelegramController<TContext> where TContext : TelegramContext {
    ...
    public async Task SubscribeToNewsletter() {
        ...
        if (newsletters.Any(n => n.Key == newsletterKey)) {
            var result = await newsletterService.SubscribeChatAsync(newsletterKey, TelegramChat!.Id);
            if (result) {
    ...
    public async Task UnsubscribeFromNewsletter() {
        ...
        if (newsletters.Any(n => n.Key == newsletterKey)) {
            if (await newsletterService.IsChatSubscribedToNewsletterAsync(newsletterKey, TelegramChat!.Id)) {
                var result = await newsletterService.SubscribeChatAsync(newsletterKey, TelegramChat.Id);
                if (result) {
                    await ReplyTextMessageAsync(
    ...
    public async Task SendNewsletter() {
        ...
        var newsletterKey = MessageCommand.Parameters[0];
        var newsletter = await newsletterService.GetNewsletterByKeyAsync(newsletterKey);
    ...
    public async Task SendNewsletterGetText() {
        ...
        if (newsletterKey is not null) {
            newsletter = await newsletterService.GetNewsletterByKeyAsync(newsletterKey);
        }

        Func<TelegramChat, Task> sendFunction;
        switch (Update.Message?.Type) {
            case MessageType.Text:
                sendFunction = async chat => await BotData.Bot.SendTextMessageAsync(chat.Id, Update.Message.Text!, parseMode: ParseMode.Html);
                break;
        ...
        if (newsletter != null) {
            ...
            var result = await newsletterService.SendNewsletterAsync(newsletterKey!, sendFunction);
            ...
            var result = await newsletterService.SendNewsletterAsync(sendFunction);
            await ReplyTextMessageAsync("Finished - report:\n" +
                                        $"Successful delivery: {result.TotalSuccesses}\n" +
                                        $"Failed delivery: {result.TotalErrors}");
            TelegramChat.State = null;
